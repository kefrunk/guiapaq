  <h1><?php echo Yii::t('guia','Playas')?></h1> 
<p><?php echo Yii::t('guia','intro playas Elche')?></p>
<p><iframe class="video" src="http://www.youtube.com/embed/g4PBYTdtfLs" frameborder="0" allowfullscreen></iframe></p>
<p><?php echo Yii::t('guia','intro playas Elche 2')?></p>
<h2>Arenales del Sol</h2> 
<p> 	
		<img src="/imgGuia/playabierta.gif" title="<?php echo Yii::t('guia','Playas')?>">
		<img src="/imgGuia/playasemiurb.gif" title="<?php echo Yii::t('guia','imgAltPlayaSemiurbana')?>">
		<img src="/imgGuia/estadomar.gif" alt="<?php echo Yii::t('guia','imgAltEstadodelmar')?>" title="<?php echo Yii::t('guia','imgAltEstadodelmar')?>">
		<img src="/imgGuia/Pasarelas.gif" alt="<?php echo Yii::t('guia','imgAltPasarelas')?>" title="<?php echo Yii::t('guia','imgAltPasarelas')?>">
		<img src="/imgGuia/accminus.gif" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>">
		<img src="/imgGuia/Restaurante.gif" alt="<?php echo Yii::t('guia','imgAltRestaurante')?>" title="<?php echo Yii::t('guia','imgAltRestaurante')?>">
		<img src="/imgGuia/Vela.gif" alt="<?php echo Yii::t('guia','imgAltVela')?>" title="<?php echo Yii::t('guia','imgAltVela')?>">
		<img src="/imgGuia/lavapie.gif" alt="<?php echo Yii::t('guia','imgAltLavapies')?>" title="<?php echo Yii::t('guia','imgAltLavapies')?>">
		<img src="/imgGuia/serviwc.gif" alt="Servicios WC" title="Servicios WC">
		<img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>">
		<img src="/imgGuia/infotur.gif" alt="Tourist Info" title="Tourist Info">
		<img src="/imgGuia/parking.gif" alt="Parking" title="Parking">
		<img src="/imgGuia/windsurf.gif" alt="Windsurf" title="Windsurf">
		<img src="/imgGuia/accesible.gif" alt="Accesible" title="Accesible">
</p> 
<p><?php echo Yii::t('guia','intro arenales')?></p>
<img src="/imgGuia/arenales-del-sol-001.jpg" width="185" height ="123px"> 
<p><?php echo Yii::t('guia','intro arenales 2')?></p>
<h2>Santa Pola</h2> 
<h3> Gran Playa </h3> 
<p> 	
		<img src="/imgGuia/playabierta.gif" title="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>">
		<img src="/imgGuia/playasemiurb.gif" title="Playa Semi Urbana"> 
		<img src="/imgGuia/estadomar.gif" title="<?php echo Yii::t('guia','imgAltEstadodelmar')?>"> 
		<img src="/imgGuia/Pasarelas.gif" title="<?php echo Yii::t('guia','imgAltPasarelas')?>" > 
		<img src="/imgGuia/accminus.gif" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>"> 
		<img src="/imgGuia/restaurante.gif" title="<?php echo Yii::t('guia','imgAltRestaurante')?>" > 
		<img src="/imgGuia/lavapie.gif" title="<?php echo Yii::t('guia','imgAltLavapies')?>" > 
		<img src="/imgGuia/chiringuito.gif" title="Chiringuito"> 
		<img src="/imgGuia/cruzroja.gif" title="Cruz Roja"> 
		<img src="/imgGuia/serviwc.gif" title="Servicios WC"> 
		<img src="/imgGuia/deporti.gif" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>">
		<img src="/imgGuia/infotur.gif" title="Tourist Info"> 
		<img src="/imgGuia/parking.gif" title="Parking"> 
		<img src="/imgGuia/windsurf.gif" title="Windsurf"> 
		<img src="/imgGuia/patines.gif" title="<?php echo Yii::t('guia','imgAltPatines')?>"> 
		<img src="/imgGuia/accesible.gif" title="Accesible">
</p> 
<img src="/imgGuia/santapola1.jpg" width="185px" height="123px"> <img src="/imgGuia/santapola2.jpg" width="185px" height="123px"> 
<p><?php echo Yii::t('guia','intro Gran Playa')?>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-santa-pola-gran-playa-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a> </p>
<h3> Playa Levante</h3> 
<p>
<img src="/imgGuia/playabierta.gif" alt="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>">
<img src="/imgGuia/estadomar.gif" alt="<?php echo Yii::t('guia','imgAltEstadodelmar')?>" title="<?php echo Yii::t('guia','imgAltEstadodelmar')?>"> 
<img src="/imgGuia/pasarelas.gif" alt="<?php echo Yii::t('guia','imgAltPasarelas')?>" title="<?php echo Yii::t('guia','imgAltPasarelas')?>"> 
<img src="/imgGuia/accminus.gif" alt="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>"> 
<img src="/imgGuia/restaurante.gif" alt="<?php echo Yii::t('guia','imgAltRestaurante')?>" title="<?php echo Yii::t('guia','imgAltRestaurante')?>"> 
<img src="/imgGuia/lavapie.gif" alt="<?php echo Yii::t('guia','imgAltLavapies')?>" title="<?php echo Yii::t('guia','imgAltLavapies')?>"> 
<img src="/imgGuia/chiringuito.gif" alt="Chiringuito" title="Chiringuito">
 <img src="/imgGuia/cruzroja.gif" alt="Cruz Roja" title="Cruz Roja">
 <img src="/imgGuia/serviwc.gif" alt="Servicios WC" title="Servicios WC">
 <img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>">
 <img src="/imgGuia/playaurban.gif" alt="Playa Urbana" title="Playa Urbana"> 
 <img src="/imgGuia/hoteles.gif" alt="Hoteles" title="Hoteles"> 
 <img src="/imgGuia/parking.gif" alt="Parking" title="Parking">
 <img src="/imgGuia/patines.gif" alt="<?php echo Yii::t('guia','imgAltPatines')?>" title="<?php echo Yii::t('guia','imgAltPatines')?>"> 
 <img src="/imgGuia/accesible.gif" alt="Accesible" title="Accesible">
 <img src="/imgGuia/bandera_azul.gif" alt="<?php echo Yii::t('guia','imgAltBanderazul')?>" title="<?php echo Yii::t('guia','imgAltBanderazul')?>"></p> 
<img src="/imgGuia/santapola3.jpg" width="185px" height="123px"> <img src="/imgGuia/santapola6.jpg" width="185px" height="123px"> 
<p><?php echo Yii::t('guia','intro Playa Levante')?>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-santa-pola-playa-llevant-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a> </p>
<h3> Calas </h3> 
<p> <img src="/imgGuia/cala.gif" alt="Cala" title="Cala"> 
<img src="/imgGuia/chiringuito.gif" alt="Chiringuito" title="Chiringuito"> 
<img src="/imgGuia/playavirgen.gif" alt="<?php echo Yii::t('guia','imgAltPlayavirgen')?>" title="<?php echo Yii::t('guia','imgAltPlayavirgen')?>"> 
<img src="/imgGuia/motoagua.gif" alt="<?php echo Yii::t('guia','imgAltMotoagua')?>" title="<?php echo Yii::t('guia','imgAltMotoagua')?>">
</p> 
<img src="/imgGuia/cabo_aljup.jpg"> 
<img src="/imgGuia/cala1.jpg" width="185px" height="123px">
<img src="/imgGuia/cala2.jpg" width="185px" height="123px"> 
<p><?php echo Yii::t('guia','intro Calas Santa Pola')?>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-santa-pola-calas-santa-pola-del-este-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a></p>
<h3> Playa Varadero </h3> <p> 
<img src="/imgGuia/playabierta.gif" alt="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>" title="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>"> 
<img src="/imgGuia/playasemiurb.gif" alt="Playa Semi Urbana" title="Playa Semi Urbana"> 
<img src="/imgGuia/pasarelas.gif" alt="<?php echo Yii::t('guia','imgAltPasarelas')?>" title="<?php echo Yii::t('guia','imgAltPasarelas')?>"> 
<img src="/imgGuia/accminus.gif" alt="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>"> 
<img src="/imgGuia/restaurante.gif" alt="<?php echo Yii::t('guia','imgAltRestaurante')?>" title="<?php echo Yii::t('guia','imgAltRestaurante')?>"> 
<img src="/imgGuia/lavapie.gif" alt="<?php echo Yii::t('guia','imgAltLavapies')?>" title="<?php echo Yii::t('guia','imgAltLavapies')?>"> 
<img src="/imgGuia/chiringuito.gif" alt="Chiringuito" title="Chiringuito"> 
<img src="/imgGuia/cruzroja.gif" alt="Cruz Roja" title="Cruz Roja"> 
<img src="/imgGuia/serviwc.gif" alt="Servicios WC" title="Servicios WC"> 
<img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>"> 
<img src="/imgGuia/infotur.gif" alt="Tourist Info" title="Tourist Info"> 
<img src="/imgGuia/parking.gif" alt="Parking" title="Parking"> <img src="/imgGuia/patines.gif" alt="Patines" title="Patines"> <img src="/imgGuia/bandera_azul.gif" alt="<?php echo Yii::t('guia','imgAltBanderazul')?>" title="<?php echo Yii::t('guia','imgAltBanderazul')?>"> </p> 
<img src="/imgGuia/santapola4.jpg" width="185px" height="123px"> <img src="/imgGuia/santapola5.jpg" width="185px" height="123px"> 
<p><?php echo Yii::t('guia','intro Varadero')?>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-santa-pola-playa-varadero-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a></p>
<h3>Playa Tamarit </h3> 
<p> <img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>"> 
<img src="/imgGuia/playabierta.gif" alt="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>" title="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>"> 
<img src="/imgGuia/pasarelas.gif" alt="<?php echo Yii::t('guia','imgAltPasarelas')?>" title="<?php echo Yii::t('guia','imgAltPasarelas')?>"> 
<img src="/imgGuia/accminus.gif" alt="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>">
 <img src="/imgGuia/restaurante.gif" alt="<?php echo Yii::t('guia','imgAltRestaurante')?>" title="<?php echo Yii::t('guia','imgAltRestaurante')?>"> 
 <img src="/imgGuia/vela.gif" alt="<?php echo Yii::t('guia','imgAltVela')?>" title="<?php echo Yii::t('guia','imgAltVela')?>"> 
 <img src="/imgGuia/chiringuito.gif" alt="Chiringuito" title="Chiringuito"> <img src="/imgGuia/cruzroja.gif" alt="Cruz Roja" title="Cruz Roja"> 
 <img src="/imgGuia/serviwc.gif" alt="Servicios WC" title="Servicios WC"> 
 <img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>"> 
 <img src="/imgGuia/playaurban.gif" alt="Playa Urbana" title="Playa Urbana"> <img src="/imgGuia/parking.gif" alt="Parking" title="Parking"> 
 <img src="/imgGuia/patines.gif" alt="Patines" title="Patines"> <img src="/imgGuia/accesible.gif" alt="Accesible" title="Accesible"> 
 <img src="/imgGuia/bandera_azul.gif" alt="<?php echo Yii::t('guia','imgAltBanderazul')?>" title="<?php echo Yii::t('guia','imgAltBanderazul')?>"> </p> 
<img src="/imgGuia/santapola7.jpg" width="185px" height="123px"> <img src="/imgGuia/santapola8.jpg" width="185px" height="123px">
<p><?php echo Yii::t('guia','intro Tamarit')?>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-santa-pola-playa-tamarit-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a></p>
<h2>La Marina</h2> 
<img src="/imgGuia/lamarina1.jpg" width="185px" height="123px"> <img src="/imgGuia/lamarina2.jpg" width="185px" height="123px">
<p><?php echo Yii::t('guia','intro La Marina')?>
<p> <img src="/imgGuia/windsurf.gif" alt="Windsurf" title="Windsurf"/> 
<img src="/imgGuia/parking.gif" alt="Parking" title="Parking"/> 
<img src="/imgGuia/hoteles.gif" alt="Hoteles" title="Hoteles"/> 
<img src="/imgGuia/infotur.gif" alt="Tourist Info" title="Tourist Info"/> 
<img src="/imgGuia/deporti.gif" alt="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>" title="<?php echo Yii::t('guia','imgAltJuegosDeportivos')?>"/> 
<img src="/imgGuia/serviwc.gif" alt="Servicios WC" title="Servicios WC"/>
 <img src="/imgGuia/playavirgen.gif" alt="<?php echo Yii::t('guia','imgAltPlayavirgen')?>" title="<?php echo Yii::t('guia','imgAltPlayavirgen')?>"/> 
 <img src="/imgGuia/lavapie.gif" alt="<?php echo Yii::t('guia','imgAltLavapies')?>" title="<?php echo Yii::t('guia','imgAltLavapies')?>"/> 
 <img src="/imgGuia/vela.gif" alt="<?php echo Yii::t('guia','imgAltVela')?>" title="<?php echo Yii::t('guia','imgAltVela')?>"/> 
 <img src="/imgGuia/restaurante.gif" alt="<?php echo Yii::t('guia','imgAltRestaurante')?>" title="<?php echo Yii::t('guia','imgAltRestaurante')?>"/>
 <img src="/imgGuia/accminus.gif" alt="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>" title="<?php echo Yii::t('guia','imgAltAccesomovreducida')?>"/> 
 <img src="/imgGuia/camping.gif" alt="Camping" title="Camping"/> 
 <img src="/imgGuia/pasarelas.gif" alt="<?php echo Yii::t('guia','imgAltPasarelas')?>" title="<?php echo Yii::t('guia','imgAltPasarelas')?>"/> 
 <img src="/imgGuia/estadomar.gif" alt="<?php echo Yii::t('guia','imgAltEstadodelmar')?>" title="<?php echo Yii::t('guia','imgAltEstadodelmar')?>"/> 
 <img src="/imgGuia/playabierta.gif" alt="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>" title="<?php echo Yii::t('guia','imgAltPlayaAbierta')?>"/> 
 <img src="/imgGuia/bandera_azul.gif" alt="<?php echo Yii::t('guia','imgAltBanderazul')?>" title="<?php echo Yii::t('guia','imgAltBanderazul')?>"/> </p>
<a href="http://www.comunitatvalenciana.com/playa/costa-blanca-elx/elche-playa-la-marina-0" target="_blank"><?php echo Yii::t('guia','Mas Info')?></a> </p>
